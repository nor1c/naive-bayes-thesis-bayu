<div class="px-5 w-full mx-auto text-xs">
	...
	<div class="w-full text-right mt-12">
		<a href="<?=site_url('preview/penggabungan_data_after_mapel')?>" class="px-5 py-3 text-xs font-bold rounded-lg bg-blue-500 text-white hover:bg-blue-700 hover:shadow-xl">PENGGABUNGAN DATA KEMBALI <i class="fa-solid fa-circle-right ml-1"></i></a>
	</div>
</div>
...
<script>
	$(document).ready(function() {
		...
		table = $('#previewTable').DataTable({
			...
			"scrollCollapse": true,
            "ajax": {
                "url": "<?=site_url('preview/data_hasil_pengecekan_mapel_dapodik')?>",
                'method': 'POST',
                'data': function(d) {
                    d.draw = d.draw || 1
