<div class="px-5 w-full mx-auto text-xs">
	<ol class="mb-10 flex items-center justify-center w-full text-xs font-medium text-center text-gray-500 bg-white border border-gray-200 rounded-lg shadow-sm sm:text-base sm:p-4 sm:space-x-4 rtl:space-x-reverse">
		...
		<li class="flex items-center text-sm text-blue-500">
			<span class="flex items-center justify-center w-5 h-5 me-2 text-xs border border-blue-500 rounded-full shrink-0">3</span>
			Transformasi Data
			<svg class="w-2 h-2 ms-2 sm:ms-4 rtl:rotate-180" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 12 10">
				<path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m7 9 4-4-4-4M1 9l4-4-4-4"/>
			</svg>
	...
	<h3 class="text-lg font-medium">3. Transformasi Data Kriteria (X)</h3>
	...
	<div class="w-full text-right mt-12">
		<a href="<?=site_url('thesis/persiapan_training')?>" class="px-5 py-3 text-xs font-bold rounded-lg bg-blue-500 text-white hover:bg-blue-700 hover:shadow-xl">PERSIAPAN DATA TRAINING <i class="fa-solid fa-circle-right ml-1"></i></a>
	</div>
</div>
...
<script>
	$(document).ready(function() {
		...
		table = $('#previewTable').DataTable({
			...
			"scrollCollapse": true,
            "ajax": {
                "url": "<?=site_url('thesis/data_prepare_training')?>",
                'method': 'POST',
                'data': function(d) {
                    d.draw = d.draw || 1
